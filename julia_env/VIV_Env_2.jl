...
function init_env(; step=0, px=0.0f0, py=0.0f0, vx=0.0f0, vy=0.0f0, D=16)
    ...
    VIVSimEnv(sim, infos, trigger, D, step, vx, vy, px, py, px_ref, py_ref, vx_ref, vy_ref, t_ref)
end
...
function step!(env::VIVSimEnv, input_vx::Real; render=false)
    done = false
    img = nothing
    img_np = nothing
    ...
    while env.trigger == false
        ...
        if (env.step >= 750) && (abs(env.py) <= 0.5)
            env.trigger = true
        end
    end
    ...
    push!(env.infos, info)
    reward = -abs(env.py) / env.D + (env.step - 770) / 730
